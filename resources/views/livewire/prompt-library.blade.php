        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <textarea wire:model="content" rows="4"
                    class="orbit-input w-full"></textarea>
            ...
            <x-ai-orbit::card>
                <div class="flex items-start justify-between">
                    <div class="flex-1 min-w-0">
                        <h4 class="text-sm font-semibold text-gray-900 dark:text-gray-50">{{ $prompt->name }}</h4>
                        @if ($prompt->agent_class)
                            <p class="mt-0.5 text-xs text-gray-400 dark:text-gray-500 font-mono">{{ $prompt->agent_class }}</p>
                        @endif
                        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400 line-clamp-3">{{ $prompt->content }}</p>
                        @if (!empty($prompt->tags))
                            <div class="flex flex-wrap gap-1 mt-2">
                                @foreach ($prompt->tags as $tag)
                                    <span class="px-2 py-0.5 rounded text-xs bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400">{{ $tag }}</span>
                                @endforeach
                            </div>
                        @endif
                    </div>
