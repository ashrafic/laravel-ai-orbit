            ...
            @foreach ($messages as $message)
                @php
                    $roleStyles = match ($message->role) {
                        'user' => 'ml-auto bg-gradient-to-br from-orbit-500 to-orbit-600 text-white',
                        'assistant' => 'mr-auto glass-card text-gray-900 dark:text-gray-50',
                        'system' => 'mx-auto glass-card border-yellow-200/50 dark:border-yellow-800/50 text-yellow-800 dark:text-yellow-200',
                        'tool' => 'mr-auto glass-card border-purple-200/50 dark:border-purple-800/50 text-purple-800 dark:text-purple-200',
                        default => 'glass-card text-gray-900 dark:text-gray-50',
                    };
                ...
                <div class="max-w-2xl {{ $roleStyles }} rounded-xl p-4 {{ $message->role === 'user' ? 'ml-auto' : ($message->role === 'system' ? 'mx-auto text-center max-w-lg' : '') }}">
                    {{-- Message Header --}}
                    ...
                    @if (!empty($message->tool_calls) && $message->tool_calls !== 'null')
                        ...
                        @foreach ($toolCalls as $index => $toolCall)
                            <details class="mt-2">
                                ...
                                <div class="mt-2 p-3 glass-card text-xs font-mono">
                    ...
                    @if ($showRawPayload)
                        <details class="mt-2">
                            ...
                            <pre class="mt-2 p-3 glass-card text-xs overflow-x-auto">{{ json_encode($message, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}</pre>
                        </details>
                    @endif
                </div>
            @endforeach
